...
namespace Complete
{
    public class TankShooting : MonoBehaviourPunCallbacks
    {
        ...
        private void Fire()
        {
            ...
            photonView.RPC("FireOther", RpcTarget.Others, m_FireTransform.position, m_FireTransform.rotation, m_CurrentLaunchForce);
        ...
        [PunRPC]
        private void FireOther(Vector3 pos, Quaternion rotate,  float force)
        {
            //作業:砲彈掉落點一樣(將力與發射)
            //Rigidbody Shell = Instantiate(m_Shell, m_FireTransform.position, m_FireTransform.rotation) as Rigidbody;
            m_Shell.position = pos;
            m_Shell.rotation = rotate;
            m_Shell.velocity = force * m_FireTransform.forward;            
        }
    }
}